...
import argparse
from model.top_net import TopNet
...
def parse_args():
    # PARAMETERS
    parser = argparse.ArgumentParser('top')
...
def main(p_args):
    ...
    h = History('top', './history', p_args.epoch, p_args.batch_size, info='_1155')
    ...
    for epoch in range(p_args.epoch):
        ...
        for batch_id, (p, c) in enumerate(train_data_loader, 0):
            ...
            # loss function
            loss = cd_loss(dense_pred, c)
            # loss1 = emd_loss(dense_pred, c)
            #
            # loss = loss + loss1
    ...
    # 保存网络模型
    torch.save(model.cpu().state_dict(), './model_save/top_1155.pth')
    h.save_history()
